...
def generate_json
  json_str = `/Applications/calibre.app/Contents/MacOS/calibredb list --for-machine --fields authors,title`
  JSON.parse(json_str)
end

def generate_books(books_dir)
      # @opts   = opts
      @dir    = File.join [books_dir].compact
